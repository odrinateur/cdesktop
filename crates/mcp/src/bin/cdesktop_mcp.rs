...
fn main() -> anyhow::Result<()> {
    ...
    tokio::runtime::Builder::new_multi_thread()
        ...
        .block_on(async move {
            ...
            init_process_logging("cdesktop-mcp", version);

            let base_url = resolve_base_url("cdesktop-mcp").await?;
    ...
    I: Iterator<Item = String>,
{
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "-h" | "--help" => {
                println!("Usage: cdesktop-mcp --mode <global|orchestrator>");
                std::process::exit(0);
            }
            _ => {
                return Err(anyhow::anyhow!(
                    "Unknown argument '{arg}'. Usage: cdesktop-mcp --mode <global|orchestrator>"
                ));
            }
        }
    }
...
async fn resolve_base_url(log_prefix: &str) -> anyhow::Result<String> {
    if let Ok(url) = std::env::var("CDESKTOP_BACKEND_URL") {
        tracing::info!(
            "[{}] Using backend URL from CDESKTOP_BACKEND_URL: {}",
            log_prefix,
            url
        );
        return Ok(url);
        ...
        .or_else(|_| std::env::var("PORT"))
    {
        ...
        Err(_) => {
            let port = read_port_file("cdesktop").await?;
...
fn init_process_logging(log_prefix: &str, version: &str) {
    ...
    tracing::debug!(
        "[{}] Starting cdesktop MCP server version {}...",
        log_prefix,
        version
    );
}
