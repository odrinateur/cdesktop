...
async function installAndLaunchWindows(
  ...
  const sentinel = readSentinel(dir);
  if (sentinel?.appPath) {
    const appExe = path.join(sentinel.appPath, 'cdesktop.exe');
  ...
  console.error('Installing cdesktop...');
  try {
    ...
    if (fs.existsSync(path.join(defaultDir, 'cdesktop.exe'))) {
      writeSentinel(dir, {
        type: 'nsis-exe',
        appPath: defaultDir,
      });
      return launchWindowsApp(
        path.join(defaultDir, 'cdesktop.exe')
      );
    }
    console.error(
      'Installation complete. Please launch cdesktop from your Start menu.'
    );
    return 0;
  }
  ...
  const appExe = path.join(installDir, 'cdesktop.exe');
  if (fs.existsSync(appExe)) {
    return launchWindowsApp(appExe);
  }
...
function launchWindowsApp(appExe: string): number {
  console.error('Launching cdesktop...');
